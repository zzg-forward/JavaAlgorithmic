...
public class T29MoreThanHalfNum {
    public static void main(String[] args) {
        System.out.println(moreThanHalfNum(new int[]{1,1,1,1,1,1,2,2,2,2}));
    }

    private static int moreThanHalfNum(int[] testArray) {
        int result = testArray[0], count = 1;
        for (int i = 0; i < testArray.length; i++) {
            if (testArray[i] == result) count++;
            else count--;
            if (count == 0) {
                result = testArray[i];
                count = 0;
        ...
        count = 0;
        for (int i = 0; i < testArray.length; i++) {
            if (result == testArray[i]) count++;
        }

        return count > testArray.length / 2 ? result : 0;
